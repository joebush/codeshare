...
var Room = {
	...
	get: function(name, callback){
		...
		mysqlPool.getConnection(function(connErr, connection) {
			...
			connection.query("SELECT * FROM rooms WHERE name =?",
				[name], function(queryErr, rows, fields) {
				if (queryErr) throw queryErr;
				var room = null;
				if(rows[0]){
					room = rows[0];
	...
	get_messages: function(room_id, callback){
		mysqlPool.getConnection(function(connErr, connection) {
			connection.query("SELECT * FROM messages WHERE room_id = ?",
				[room_id], function(queryErr, rows, fields) {
				if (queryErr) throw queryErr;
...
var Message = {
	...
	save: function(room_id, message, type, username, ip){
		...
		mysqlPool.getConnection(function(connErr, connection) {
			...
			connection.query("INSERT INTO messages (room_id, message, type, username, ip_address, date_created) VALUES (?, ?, ?, ?, ?, NOW())",
				[room_id, message, type, username, ip], function(queryErr, rows, fields) {
				if (queryErr) throw queryErr;
	...
	Room.get(roomName, function(room){
		...
		if(!room){
			Room.save(roomName, 'Jordan', function(roomId){
				response.render('index.html', {room_id: roomId}, function(err, html){
					response.send(html);
				});
			});
